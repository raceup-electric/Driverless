...
public:
    ...
    GraphSLAMReal(const rclcpp::NodeOptions &options) : Node("graph_slam_real", options)
    {
        ...
        timer_optim = this->create_wall_timer(908s, std::bind(&GraphSLAMReal::timer_callback, this), timer_cb_group); // TODO: handle callback group
...
private:
    ...
    void sync_callback(eufs_msgs::msg::ConeArrayWithCovariance::SharedPtr cones_msg, eufs_msgs::msg::CarState::SharedPtr odom_msg)
    {
        ...
        if (cones_msg->big_orange_cones.size() >= 2 && time_diff.count() > 90.0 && !LOOP_CLOSED) //&& time_diff.count() > 90.0 && !LOOP_CLOSED
        {
