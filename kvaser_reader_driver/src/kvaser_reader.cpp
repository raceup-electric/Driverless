...
public:
    ...
    KvaserReader(const rclcpp::NodeOptions &options) : Node("kvaser_reader", options)
    {
        ...
        can_pub = this->create_publisher<kvaser_reader_driver::msg::CanData>("can_data_out", 1); // topic with CAN data, not parsed
...
private:
    rclcpp::Publisher<kvaser_reader_driver::msg::CanData>::SharedPtr can_pub;
    ...
    // tire model: gets lateral force as input and returns ?
    void tireSlip (double force) {
    ...
    void dumpMessageLoop(canHandle hnd, int channel_number)
    {
        ...
        while (rclcpp::ok())
        {
            ...
            kvaser_reader_driver::msg::CanData can_msg;
